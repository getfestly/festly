...
const VEHICLE_HINTS = {
  pkw_anhaenger: 'Anlieferungshinweis: Das Angebot wird per PKW mit Anhänger angeliefert. Bitte stellen Sie sicher, dass die Zufahrt für PKW mit Anhänger befahrbar und ausreichend Rangierplatz vorhanden ist.',
  lkw_mittel:    'Anlieferungshinweis: Das Angebot wird per LKW (ab 7,5t) angeliefert. Bitte stellen Sie sicher, dass die Zufahrt für LKW befahrbar, das Gelände tragfähig und ausreichend Rangierplatz vorhanden ist.',
  lkw_gross:     'Anlieferungshinweis: Das Angebot wird per LKW mit Kran (ab 7,5t) angeliefert. Bitte stellen Sie sicher, dass: die Zufahrt für Schwerlastfahrzeuge befahrbar ist, der Kranstandplatz verdichtet und tragfähig ist, ausreichend Rangier- und Aufbaufläche vorhanden ist und keine Hindernisse (Leitungen, Bäume, Bauten) den Kranbereich einschränken.',
}

function vehicleHintBlock(vehicle_type) {
  const hint = VEHICLE_HINTS[vehicle_type]
  if (!hint) return ''
  return `<div style="margin-top:20px;padding:14px 16px;background:#fffbeb;border:1px solid #fde68a;border-radius:10px;">
    <p style="margin:0;color:#92400e;font-size:13px;line-height:1.6;">⚠️ ${hint}</p>
  </div>`
}

/** Kunde: Buchung bestätigt */
export async function sendBookingAccepted({ to, customerName, listingTitle, providerName, eventDate, amount_cents, bookingId, vehicle_type }) {
  const resend = getResend()
  await resend.emails.send({
    from: FROM,
    to,
...
export async function sendBookingRejected({ to, customerName, listingTitle }) {
  ...
  await resend.emails.send({
    ...
    html: base(`
      ...
      ${btn('Marktplatz erkunden', `${APP_URL}/`)}
    `),
  })
}
