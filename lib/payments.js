...
// ---------------------------------------------------------------------------
// Storno: gestaffelte Rückerstattung nach Vorlauf
// ---------------------------------------------------------------------------
export function calcRefundCents(amountCents, eventDate, cancelledBy) {
  if (cancelledBy === 'provider') return amountCents  // Anbieter trägt vollen Schaden

  const daysUntil = Math.floor(
    (new Date(eventDate) - new Date()) / (1000 * 60 * 60 * 24)
  )
  if (daysUntil >= 30) return amountCents
  if (daysUntil >= 14) return Math.round(amountCents * 0.5)
  if (daysUntil >= 3)  return Math.round(amountCents * 0.25)
  return 0
}
...
export async function cancelBooking(bookingId, cancelledBy) {
  ...
  const refundCents = booking.status === 'paid'
    ? calcRefundCents(booking.amount_cents, booking.event_date, cancelledBy)
    : 0
  const feeCents = booking.amount_cents - refundCents
