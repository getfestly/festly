...
export async function transferToProvider(bookingId) {
  ...
  // Buchung + Anbieter-Stripe-Konto laden
  const { data: booking } = await admin
    .from('bookings')
    .select(`
      id, amount_cents, provider_payout_cents,
  ...
  // Charge-ID aus Payment Intent holen (source_transaction braucht die Charge, nicht den PI)
  ...
  // Auszahlungsbetrag: gespeicherter Wert oder 85 % Fallback
  ...
  // DB: Zahlung als "ausgezahlt" markieren
  ...
  // DB: Buchung als "abgeschlossen" markieren
