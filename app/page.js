    ...
    <section>
      ...
      {category.listings.length === 0 ? (
        <div className="bg-gray-50 rounded-2xl p-8 text-center">
          ...
          <Link href="/anbieter/listings/neu"
...
function HomeContent() {
  ...
  // Aktive Kategorie aus URL (?kategorie=fahrgeschaefte)
  // Wird durch Nav-Tabs und Suchleiste gesetzt
  ...
  // Listings laden (einmalig — Filterung passiert client-seitig)
  useEffect(() => {
    async function load() {
      try {
  ...
  // Panels schließen bei Klick außerhalb
  useEffect(() => {
    function handler(e) {
  ...
  // Kategorie in URL setzen (soft navigation, kein Reload)
  ...
  // Suche → Marktplatz mit Parametern
  function handleSearch() {
    const p = new URLSearchParams()
    if (activeCategory) p.set('kategorie', activeCategory)
  ...
  // Falls eine Kategorie aktiv: nur diese anzeigen. Sonst alle mit mind. 1 Listing.
  const grouped = KATEGORIEN
    .filter(k => !activeCategory || k.id === activeCategory)
    .map(k => ({ ...k, listings: listings.filter(l => l.category === k.id) }))
    .filter(k => !activeCategory ? k.listings.length > 0 : true)

  const regions          = Object.values(REGION_NAMES).sort((a, b) => a.localeCompare(b, 'de'))
  const selectedCatLabel = KATEGORIEN.find(k => k.id === activeCategory)?.label ?? null
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-10">
            <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-900 leading-tight tracking-tight mb-3">
              Dein Event.{' '}<span className="gradient-text">Perfekt organisiert.</span>
            </h1>
          ...
          <div ref={searchRef} className="relative max-w-3xl mx-auto">
              ...
              <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 max-h-72 overflow-y-auto">
                <div className="p-2">
                  <button onClick={() => setCategory('')}
                    className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors text-left ${!activeCategory ? 'bg-gray-50' : 'hover:bg-gray-50'}`}>
                    ...
                    {!activeCategory && <span className="ml-auto text-gray-900">✓</span>}
                  </button>
                  {KATEGORIEN.map(k => (
                    <button key={k.id} onClick={() => setCategory(k.id)}
                      className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors text-left ${activeCategory === k.id ? 'bg-gray-50' : 'hover:bg-gray-50'}`}>
                      ...
                      {activeCategory === k.id && <span className="ml-auto text-gray-900">✓</span>}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {activeField === 'where' && (
              <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 max-h-72 overflow-y-auto">
                <div className="p-2">
                  <button onClick={() => { setSelectedRegion(''); setActiveField(null) }}
                  ...
                  {regions.map(r => (
                    <button key={r} onClick={() => { setSelectedRegion(r); setActiveField(null) }}
